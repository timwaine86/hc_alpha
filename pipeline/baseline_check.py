...
FORCE_CREATE        = os.getenv("NOTION_CREATE_IF_MISSING") == "1"

# ---------- OpenAI territory classifier ----------
TERRITORY_LABELS = [
...
def _fewshot_from_existing(df):
    """Build a light few-shot from rows that already have Assigned_Territory_v4."""
    shots = []
...
def classify_missing_territories(df):
    """Return df with new column Assigned_Territory_v4_pred; fill v4 if missing (last 60d only)."""
    try:
        from openai import OpenAI
    ...
    for idx in df[missing].index:
        cap = str(
            df.at[idx, "caption_v4"]
            if "caption_v4" in df.columns else
            df.at[idx, "caption_v3"]
            if "caption_v3" in df.columns else
            df.at[idx, "caption"]
            if "caption" in df.columns else
            ""
        )
        fmt = str(
            df.at[idx, "format_v4"]
            if "format_v4" in df.columns else
            df.at[idx, "format_v3"]
            if "format_v3" in df.columns else
            df.at[idx, "format"]
            if "format" in df.columns else
            ""
        )
        try:
...
if VIEWS_COL:
    df["views"] = df[VIEWS_COL].apply(to_num)
...
# ---------- Territory breakdown (30d, ER median, no outliers) ----------
cand_cols = [
    "Assigned_Territory_v4",
    "Assigned_Territory_v3",
    "Assigned_Territory",
    "Assigned_Territory_v3_v4",
    "Assigned_Territory_v2_v4",
]
ter_col = next((c for c in cand_cols if c in latest_no_outliers.columns), None)
if ter_col:
    g = latest_no_outliers.groupby(ter_col)["ER"].median().sort_values(ascending=False)
    print("\nTop territories by ER median (30d, no outliers):")
    for k, v in g.head(3).items():
        print(f" - {k}: {v:.2f}%")
    print("\nBottom territories by ER median (30d, no outliers):")
    for k, v in g.tail(3).items():
        print(f" - {k}: {v:.2f}%")
else:
    print("\n(No territory column found for breakdown)")
...
from datetime import date, timedelta  # make sure this import is present at top

def notion_query_today_by_type(type_name: str):
    """
    Return the first page within a ±1 day window for the given Insight Type,
    or None if no page matches. Never index into an empty list.
    """
    if not NOTION_TOKEN or not DB_ID:
        return None
    ...
    body = {
        "filter": {
            "and": [
                ...
                {"property": "Date", "date": {"on_or_after": (date.today() - timedelta(days=1)).isoformat()}},
                {"property": "Date", "date": {"on_or_before": (date.today() + timedelta(days=1)).isoformat()}},
            ]
        },
        "page_size": 1,
    }
...
# ---------- Update-or-create (DEFENSIVE) ----------
force_create = os.getenv("NOTION_CREATE_IF_MISSING") == "1"
existing_pulse = None if force_create else notion_query_today_by_type("Weekly Pulse")

if existing_pulse is not None and existing_pulse.get("id"):
...
if pd.notna(ivr_delta) and ivr_delta < 0:
    ...
    existing_pred = None if FORCE_CREATE else notion_query_today_by_type("Predictive Test")
    if existing_pred is not None and existing_pred.get("id"):
